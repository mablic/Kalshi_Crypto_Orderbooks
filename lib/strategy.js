...
/**
 * Get trading interval/period from strategy details
 * @param {string} strategyName - Strategy name (e.g., 'LSTM_Strategy_A_AAPL')
 * @returns {Promise<string>} Interval in minutes (e.g., '15', '1'), defaults to '1' if not found
 */
export const getStrategyPeriod = async (strategyName) => {
  try {
    const strategyDetails = await getStrategyDetails(strategyName);
    
    if (strategyDetails) {
      // Check both 'period' and 'Period' (case-insensitive)
      const periodValue = strategyDetails.period || strategyDetails.Period;
      
      if (periodValue) {
        // Handle different formats: "15m", "15", "15 minutes", etc.
        const period = String(periodValue).trim();
        // Extract number from formats like "15m", "15", "15 minutes"
