...
export async function listKalshiMarkets() {
  ...
  rows.sort((a, b) => {
    const ma = kalshiTimeToMs(getKalshiExpirationRaw(a.data));
    const mb = kalshiTimeToMs(getKalshiExpirationRaw(b.data));
...
/**
 * Canonical settlement/expiration instant for UI and sorting.
 * Prefer Kalshi `expiration_time` (same source as Overview "Expiration Time"), then trading close, then fallbacks.
 */
export function getKalshiExpirationRaw(marketDoc) {
  if (!marketDoc || typeof marketDoc !== 'object') return null;
  return (
    marketDoc.expiration_time ??
    marketDoc.expirationTime ??
    marketDoc.close_time ??
    marketDoc.closeTime ??
    marketDoc.expected_expiration_time ??
    marketDoc.expectedExpirationTime ??
    marketDoc.end_time ??
    marketDoc.endTime ??
    null
  );
...
/**
 * User's local timezone (browser); no zone abbreviation suffix.
 * Pass Firestore Timestamp, Date, ISO string, or unix.
 */
...
/**
 * Local date + time without weekday (e.g. market ticker dropdown labels).
 */
export function formatKalshiLocalDateTimeCompact(value) {
  if (value == null || value === '') return '';
  const ms = kalshiTimeToMs(value);
  if (ms == null) return '';
  try {
...
export function parseOrderBookDepthRows(raw) {
  ...
  // Bids: higher price is more aggressive — show best bid first (descending).
...
/**
 * JSON array of [price_dollars, contracts] levels for one side of the book (CSV cell).
 * @param {Array<{ price: number, size: number }>} rows
 */
function depthRowsToBookJson(rows) {
  if (!rows.length) return '[]';
  return JSON.stringify(rows.map((r) => [r.price, r.size]));
}

/**
 * One row per snapshot: date, OHLC, yes_book, no_book.
 * Each book is a JSON array of [price, size] levels (yes_dollars / no_dollars ladders).
 *
...
export function buildKalshiCandlesOrderBookCsv(candles) {
  ...
  const header = ['date', 'open', 'high', 'low', 'close', 'yes_book', 'no_book'];
  ...
  for (const c of candles) {
    ...
    const cells = [
      ...
      escapeCsvField(depthRowsToBookJson(yesRows)),
      escapeCsvField(depthRowsToBookJson(noRows)),
    ];
